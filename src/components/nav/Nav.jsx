    ...
    <header className="nav">
      <div className="container nav__inner">
        ...
        <nav className={`nav__links ${open ? "is-open" : ""}`} aria-label="Main navigation">
          {[["About", "about"], ["Concepts", "templates"], ["Work", "projects"], ["Tech Radar", "tech-radar"], ["Experience", "experience"], ["Contact", "contact"]].map(([label, id]) => <a key={id} href={`#${id}`} onClick={close}>{label}</a>)}
        </nav>
